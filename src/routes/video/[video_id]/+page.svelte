					...
					{#if !video.isOwn}
						<div class="flex">
							<form
								...
								return async ({ result, update }) => {
										...
										if (video) video.like = video.like === true ? undefined : true;
									}
								}
							}
							>
							...
							<form
								...
								return async ({ result, update }) => {
										if (result.type !== "success") return update();
										if (video) video.like = video.like === false ? undefined : false;
									}
								}
							}
							>
						...
						{#if video.isRepostedByAuthenticatedUser}
							<form
								...
								return async ({ result, update }) => {
										if (result.type !== "success") return update();
										if (video) video.isRepostedByAuthenticatedUser = false;
									}
								}
							}
							>
						...
						{:else}
							<form
								...
								return async ({ result, update }) => {
										if (result.type !== "success") return update();
										if (video) video.isRepostedByAuthenticatedUser = true;
									}
								}
							}
							>
